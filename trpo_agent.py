...
class TRPOAgent:
  ...
  def sample_trajectories(self):
    ...
    while timesteps_so_far < self.max_timesteps:
      ...
      for _ in range(self.max_episode_length):
        ...
        action, m, action_dist = self.sample_action_from_policy(observation)
  ...
  def surrogate_loss(self, theta):
    ...
    observations_tensor = torch.cat([Variable(Tensor(observation)).unsqueeze(0) for observation in self.observations])
    prob_new = new_model(observations_tensor).gather(1, torch.cat(self.actions).unsqueeze(1)).data
    prob_old = self.policy_model(observations_tensor).gather(1, torch.cat(self.actions).unsqueeze(1)).data
  ...
  def step(self):
    ...
    print torch.cat(self.action_dists).size()
    print torch.cat(self.actions).size()
    new_p = torch.cat(self.action_dists).gather(1, torch.cat(self.actions).unsqueeze(1))
    old_p = new_p.detach()
